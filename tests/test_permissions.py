...
from redash.permissions import has_access, has_access_torun
...
class TestHasAccess(TestCase):
    ...
    def test_allows_if_user_member_in_multiple_groups(self):
        ...
        self.assertTrue(has_access_torun({1: not view_only, 2: view_only}, user, not view_only))
        ...
        self.assertTrue(has_access_torun({1: view_only, 2: view_only}, user, view_only))
        self.assertTrue(has_access({1: not view_only, 2: not view_only}, user, view_only))
        self.assertTrue(has_access_torun({1: not view_only, 2: not view_only}, user, view_only))
    ...
    def test_not_allows_if_not_enough_permission(self):
        ...
        self.assertFalse(has_access_torun({2: view_only}, user, view_only))
        self.assertFalse(has_access({2: not view_only, 1: view_only}, user, not view_only))
        self.assertFalse(has_access_torun({2: not view_only, 1: view_only}, user, not view_only))
